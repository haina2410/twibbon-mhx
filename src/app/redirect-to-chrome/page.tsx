...
function RedirectToChromeContent() {
  const searchParams = useSearchParams();
  const targetUrl = searchParams.get("target") || window.location.origin;
  const [countdown, setCountdown] = useState(5);

  useEffect(() => {
    // Try to open in Chrome using URL scheme
    const timer = setTimeout(() => {
      // Try multiple methods to open in Chrome
      const methods = [
  ...
  const handleManualRedirect = () => {
    ...
    navigator.clipboard.writeText(targetUrl).then(() => {
      alert(
        "Link copied to clipboard! Please paste it in Chrome for the best experience."
      );
    });
  };
      ...
      <div className="bg-white rounded-xl shadow-lg p-8 max-w-md w-full text-center">
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            We&apos;re redirecting you to Chrome for a better download
            experience.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-green-50 rounded-lg p-4">
            ...
            <ol className="text-sm text-green-700 text-left space-y-1">
              <li>1. Tap the menu button (⋮) in the top right</li>
              <li>2. Select &quot;Open in Chrome&quot;</li>
